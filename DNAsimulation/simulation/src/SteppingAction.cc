...
void SteppingAction::UserSteppingAction(const G4Step *step)
{
  ...
  if (flagVolume != 0)
  {
    ...
    G4int PrimaryEventNum = generatorAction->PrimaryEventNum;
    G4int PrimaryCopyNum = generatorAction->PrimaryCopyNum;
  ...
  if ((flagVolume != 3) && (dE > 0))
  {
    ...
    G4int PrimaryEventNum = generatorAction->PrimaryEventNum;
    G4int PrimaryCopyNum = generatorAction->PrimaryCopyNum;
    G4double PrimaryTime = generatorAction->PrimaryTime;
    ...
    analysisManager->FillNtupleDColumn(1, 6, step->GetPostStepPoint()->GetKineticEnergy());
        ...
        {
    analysisManager->FillNtupleIColumn(1, 7, PrimaryCopyNum);
    analysisManager->FillNtupleIColumn(1, 8, PrimaryEventNum);
    analysisManager->FillNtupleDColumn(1, 9, PrimaryTime+(step->GetTrack()->GetGlobalTime()));
        }
