                                        ...
                                        const std::vector<G4Track *> *pProducts)
{
      ...
      (GetMolecule((&trackB))->GetDefinition() == G4Cytosine::Definition()))
  {
    ...
    analysisManager->FillNtupleSColumn(2, 5, GetMolecule(radical)->GetName());
        ...
        {
    analysisManager->FillNtupleIColumn(2, 6, PrimaryCopyNum);
    analysisManager->FillNtupleIColumn(2, 7, PrimaryEventNum);
    analysisManager->FillNtupleDColumn(2, 8, PrimaryTime + DNAElement->GetGlobalTime());
        }
