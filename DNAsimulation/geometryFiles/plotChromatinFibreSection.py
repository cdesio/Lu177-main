...
def plotNucleosome(x_0,y_0,z_0, alpha, beta, gamma):
    ...
    for n in range(1,numBPperNucleosome+1):
        ...
        t = angleNucleosome*n
        R = nucleosomeRadius - DoubleHelixRadius # not - deoxyRadius because the deoxy volumes overlap due to the small radius, increased to avoid this
        a = DoubleHelixRadius
        u = anglebetweenBP*n
        h = nuclosomePitch/(2*pi)

        # Deoxyribose positions
        y1 = R*np.cos(t) + a*np.cos(t)*np.cos(u) - (h*a*np.sin(t)*np.sin(u))/np.sqrt(R**2+h**2)
        x1 = R*np.sin(t) + a*np.sin(t)*np.cos(u) + (h*a*np.cos(t)*np.sin(u))/np.sqrt(R**2+h**2)
        z1 = h*t + R*a*np.sin(u)/np.sqrt(R**2+h**2)    - nucleosomeHeight/2.

        u += pi # second helix is 180 degrees out of phase

        y2 = R*np.cos(t) + a*np.cos(t)*np.cos(u) - (h*a*np.sin(t)*np.sin(u))/np.sqrt(R**2+h**2)
        x2 = R*np.sin(t) + a*np.sin(t)*np.cos(u) + (h*a*np.cos(t)*np.sin(u))/np.sqrt(R**2+h**2)
        z2 = h*t + R*a*np.sin(u)/np.sqrt(R**2+h**2) - nucleosomeHeight/2.   
    ...
    return posSugar1_x, posSugar1_y, posSugar1_z, posSugar2_x, posSugar2_y, posSugar2_z, posBase1_x, posBase1_y, posBase1_z, posBase2_x, posBase2_y, posBase2_z
...
chromatinSpacingX= (volumeWidth*nanometer-deoxyRadius*(numRows-1))/numRows # - deoxyRadius to avoid overlap with the bounding box
chromatinHeight = volumeWidth*nanometer
chromatinSpacingY= (volumeWidth*nanometer-deoxyRadius*(numColumns-1))/numColumns #- deoxyRadius to avoid overlap with the bounding box
...
with open ('sugarPos_{}x{}_{}nm.csv'.format(numRows,numColumns, volumeWidth), 'w') as f:
    totalSugar = ((numRows*numColumns)-numColumns//2)*numLayers*6*numBPperNucleosome
        ...
        for x in range(numRows):
            ...
            for z in range(numLayers):
                for i in range(6):
                    ...
                    point = [loopRadius,0,(z-(numLayers/2-0.5))*layerSpacing] # centre in z about 0

                    theta = ((i+0.5*(z%2)+0.5*(x%2)+0.5*(y%2))*2*pi/6)
                    ...
                    histonePositions.append([point[0]+ xOffset,point[1]+ yOffset,point[2], 0, 2*pi-theta,pi/2])

                    posSugar1_x, posSugar1_y, posSugar1_z, posSugar2_x, posSugar2_y, posSugar2_z, posBase1_x, posBase1_y, posBase1_z, posBase2_x, posBase2_y, posBase2_z = plotNucleosome(point[0],point[1],point[2], ((i+0.5*z)*2*pi/6), 2*pi-theta,pi/2) # each nucleosome is rotated about its axis
                    ...
                    for j in range(len(posSugar1_x)):
                        ...
                        f.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}'.format(posSugar1_x[j],posSugar1_y[j],posSugar1_z[j],posSugar2_x[j],posSugar2_y[j],posSugar2_z[j],posBase1_x[j], posBase1_y[j], posBase1_z[j], posBase2_x[j], posBase2_y[j], posBase2_z[j]))
                        if c!=totalSugar-1:
                            f.write('\n')
                            c+=1
...
with open ('histonePositions_{}x{}_{}nm.csv'.format(numRows,numColumns, volumeWidth), 'w') as f:
    for i in range(len(histonePositions)):
        f.write('{}\t{}\t{}\t{}\t{}\t{}'.format(histonePositions[i][0],histonePositions[i][1],histonePositions[i][2],histonePositions[i][3],histonePositions[i][4],histonePositions[i][5]))
