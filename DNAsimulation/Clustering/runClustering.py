...
def runClustering(filename: str, outputFilename: str, fEMinDamage: float, fEMaxDamage: float, probIndirect: float, sugarPosFilename: str, filenamePhoton: Union[str,bool]=False, PrimaryCopyNum: int = 0):
    ...
    for i in range(entryEdepNumber):    
        tEdep.GetEntry(i)
        if (hasattr(tEdep,"PrimaryEventNum")):
            if tEdep.PrimaryCopyNum == PrimaryCopyNum:
                if tEdep.PrimaryEventNum > numEvt:
                    numEvt = tEdep.PrimaryEventNum
                if hasattr(tEdep,"PathLengthChromatin"): # older results do not have path length
                    pathLength[tEdep.PrimaryEventNum] = tEdep.PathLengthChromatin
                else:
                    pathLength[tEdep.PrimaryEventNum] = "N/A"

                if hasattr(tEdep,"PrimaryKEEntrance"): # older results do not have path length
                        meanKEperEvent[tEdep.PrimaryEventNum]=(tEdep.PrimaryKEEntrance+tEdep.PrimaryKEExit)/2 
                else:
                    meanKEperEvent[tEdep.PrimaryEventNum]="N/A"

                if tEdep.Edep_J > 0:
                    dosePerEvent[tEdep.PrimaryEventNum]=tEdep.Edep_J / (1000 * chromatinVolume)

        else: # run not from PS file
            if tEdep.EventNo > numEvt:
                numEvt = tEdep.EventNo
            if hasattr(tEdep,"PathLengthChromatin"): # older results do not have path length
                pathLength[tEdep.EventNo] = tEdep.PathLengthChromatin
            else:
                pathLength[tEdep.EventNo] = "N/A"

            if hasattr(tEdep,"PrimaryKEEntrance"): # older results do not have path length
                meanKEperEvent[tEdep.EventNo]=(tEdep.PrimaryKEEntrance+tEdep.PrimaryKEExit)/2 
            else:
                meanKEperEvent[tEdep.EventNo]="N/A"

            if tEdep.Edep_J > 0:
                dosePerEvent[tEdep.EventNo]=tEdep.Edep_J / (1000 * chromatinVolume)

    if list(meanKEperEvent.values())[0]!="N/A":
    ...
    if filenamePhoton:
        ...
        for i in range(0,entryEdepNumberPhoton):
            ...
            if tEdepPhoton.Edep_J > 0:
                dosePerEventPhoton[tEdepPhoton.EventNo]=tEdepPhoton.Edep_J / (1000 * chromatinVolume) 
    ...
    for irow in range(nentries):
        ...
        if (hasattr(input_tree,"PrimaryEventNum")):
            if input_tree.PrimaryCopyNum == PrimaryCopyNum:
                result = checkPoint([input_tree.x,input_tree.y,input_tree.z], T0,T1)
                if (result[0] != -1):
                    key = (input_tree.PrimaryEventNum,result[0],result[1])
    ...
    if filenamePhoton:
        ...
        for irow in range(nentriesPhoton):
            ...
            result = checkPoint([input_tree_photon.x,input_tree_photon.y,input_tree_photon.z], T0,T1)
            if (result[0] != -1):
    ...
    for irow in range(nentries_indirect):
        ...
        if (hasattr(input_tree_indirect,"PrimaryEventNum")):
            if input_tree_indirect.PrimaryCopyNum == PrimaryCopyNum:       
                if hasattr(input_tree_indirect,"copyNum"):
                    if np.random.rand() <= probIndirect:
                        eventsListIndirect.append(input_tree_indirect.PrimaryEventNum)
                        copyListIndirect.append(input_tree_indirect.copyNum)
                elif not hasattr(input_tree_indirect,"DNAmolecule"):
                    if np.random.rand() <= probIndirect:
                        eventsListIndirect.append(input_tree_indirect.PrimaryEventNum)
                        point = [input_tree_indirect.x,input_tree_indirect.y,input_tree_indirect.z]
    ...
    # clustering
    tempResults = clustering(numEvt,eventsListDirect,copyListDirect,strandListDirect,eventsListIndirect,copyListIndirect, strandListIndirect)
    ...
    with open(outputFilename, "w") as f:
        ...
        if filenamePhoton:
            ...
            f.write("{},{},{},{},{},{},{},{},{}\n".format(filename,energy,LET,len(pathLength.keys()),chromatinVolume,numBP,sugarPosFilename, gitHash, clusteringGitHash)) 
    ...
    with open("DSBclusterSize_{}".format(outputFilename), "w") as f:
        ...
        if filenamePhoton:
            ...
            f.write("{},{},{},{},{},{},{},{},{}\n".format(filename,energy,LET,len(events),chromatinVolume,numBP,sugarPosFilename, gitHash, clusteringGitHash)) 
        ...
        for event in events:
            if event in eventsWithClusteringResults:
                text = str([clusterSize[eventsWithClusteringResults.index(event)][1:]]).strip("[").strip("]")
            else:
                text = "0, "*49+"0"  
...
if __name__ == "__main__":
    ...
    tempResults = clustering(3,eventsListDirect,copyListDirect,strandListDirect,eventsListIndirect,copyListIndirect, strandListIndirect)
